...
const PersonDetails = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get(`https://job-b-jlk3.onrender.com/userlogin/${email}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!id) return;
      try {
        const responseId = await fetch("https://job-b-jlk3.onrender.com/resultData", {
          method: "POST",
          headers: {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("https://job-b-jlk3.onrender.com/reqbooking", {
        ...reviewData,
        userEmail,
        seekerId,
        email: user.email,
