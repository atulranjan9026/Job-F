...
const BookingStatus = () => {
    ...
    useEffect(() => {
        const fetchBookings = async () => {
            try {
                const response = await axios.get(`https://job-searching-backend-phpz.onrender.com/accept/${UserEmail}`);
        ...
        if (bookings.length > 0) {
            const fetchSeekerDetails = async () => {
                try {
                    ...
                    const responses = await Promise.all(seekerEmails.map(email => axios.get(`https://job-searching-backend-phpz.onrender.com/seeker/${email}`)));
    ...
    const handleDelete = async (salary) => {
        try {
            await axios.delete(`https://job-searching-backend-phpz.onrender.com/accept/${salary}`);
